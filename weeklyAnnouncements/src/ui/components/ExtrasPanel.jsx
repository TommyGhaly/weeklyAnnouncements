...
function fmtDate(val) {
  ...
  if (isNaN(d)) return val; // fallback for day names
  ...
  return (
    <div style={{ display: 'flex', gap: 6, marginBottom: 6, alignItems: 'center' }}>
      ...
      <input
        ...
        placeholder="Phone"
        style={{ ...field, width: 130, flex: 'none' }}
      />
...
function MDPresetForm({ initial, onSave, onCancel }) {
  ...
  const updContact = (i, k, v) => {
    const c = [...(form.contacts ?? [])]; c[i] = { ...c[i], [k]: v }; upd('contacts', c);
  };
  ...
  return (
    <div style={{ background: '#f0f8ff', border: '1.5px solid #b8d4e8', borderRadius: 10, padding: 14, marginBottom: 10 }}>
      <input value={form.name} onChange={e => upd('name', e.target.value)} placeholder="Preset name"
        style={{ ...field, fontWeight: 600, marginBottom: 10 }} autoFocus />
      ...
      <textarea value={form.notes} onChange={e => upd('notes', e.target.value)} placeholder="Default notes"
        rows={2} style={{ ...field, marginBottom: 10, resize: 'vertical' }} />
      ...
      <div style={{ marginBottom: 10 }}>
        ...
        {(form.contacts ?? []).map((c, i) => (
          <ContactRow key={i} contact={c} onChange={(k, v) => updContact(i, k, v)} onRemove={() => remContact(i)} />
        ))}
...
function MultiDayEventForm({ initial, presets, days, onSave, onCancel }) {
  ...
  const loadPreset = p => setForm(f => ({
    ...f, name: p.name, color: p.color, time: p.time,
  ...
  const dayOptions = days.map(d => ({
    value: d.date || d.day,
    label: `${d.day}${d.date ? ` (${fmtDate(d.date)})` : ''}`,
  }));
  ...
  return (
    <div style={{ background: '#fffbf0', border: '1.5px solid #e0cba8', borderRadius: 10, padding: 14, marginBottom: 10 }}>
      ...
      <input value={form.name} onChange={e => upd('name', e.target.value)} placeholder="Event name"
        style={{ ...field, fontWeight: 600, marginBottom: 10 }} autoFocus />
      ...
      <textarea value={form.notes} onChange={e => upd('notes', e.target.value)} placeholder="Notes"
        rows={2} style={{ ...field, marginBottom: 10, resize: 'vertical' }} />
...
function MultiDaySection({ events, days, onChange }) {
  ...
  useEffect(() => {
    fetchMDPresets().then(setPresets).catch(console.error);
  }, []);

  const addPreset = async p => {
    const preset = { ...createMDPreset(), ...p };
    await saveMDPreset(preset);
    setPresets(ps => [...ps, preset]);
    setCreatingPreset(false);
  };

  const editPresetFn = async p => {
    await saveMDPreset(p);
    setPresets(ps => ps.map(x => x.id === p.id ? p : x));
    setEditingPresetId(null);
  };

  const removePreset = async id => {
    await deleteMDPreset(id);
...
function AnnouncementsSection({ announcements, onChange }) {
  ...
  const upd = (i, field, val) => {
    const a = [...announcements]; a[i] = { ...a[i], [field]: val }; onChange(a);
  };
